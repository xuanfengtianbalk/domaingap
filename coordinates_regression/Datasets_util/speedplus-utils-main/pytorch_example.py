...
def evaluate(model, dataset, device, submission_writer, batch_size, real=False):
    ...
    """ Function to evaluate heads on \'validation\', \'sunlamp\' and \'lightbox\' sets, and collect pose estimations to a submission.
...
def train_model(model, scheduler, optimizer, criterion, dataloaders, device, dataset_sizes, num_epochs):
    ...
    """ Training function, looping over epochs and batches. Return the trained heads. """
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                scheduler.step()
                model.train()  # Set heads to training mode
            else:
                model.eval()
...
def main(speed_root, epochs, batch_size):
    ...
    """ Preparing the dataset for training, setting up heads, running training, and exporting submission."""
    ...
    # Getting pre-trained heads and replacing the last fully connected layer
